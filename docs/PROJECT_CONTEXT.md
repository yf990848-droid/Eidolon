...
建议下一步先确定第一阶段代码骨架并提交到 `develop`：

1. 初始化 Next.js、TypeScript 和 Tailwind CSS。
2. 建立基础目录和环境变量示例。
3. 定义 SQLite 数据模型。
4. 完成首页静态界面和三套主题变量。
5. 建立 DeepSeek 与万相的 Provider 接口。
6. 再逐步实现创作工作台和生成链路。

在开始编码前，应确认：

- 使用的 Node.js 和包管理器版本。
- 使用 Prisma、Drizzle 或其他数据库工具。
- DeepSeek 和阿里云百炼 API Key 是否已准备好。
- 第一阶段是先做静态页面原型，还是直接实现可调用模型的完整 MVP。
