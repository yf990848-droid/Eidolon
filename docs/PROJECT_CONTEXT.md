...
>
> 最后更新：2026-08-31（三种篇幅流程、Agent 日志与书架封面修复）
...
- 主分支：`main`
- 开发分支：`develop`
- 当前阶段：DeepSeek 创作链路、原创写作模式和本地文风库已实现，正式数据持久化尚未接入
- 主要用户：第一版供项目所有者个人使用
...
1. 用真实 DeepSeek 分别验证短篇整篇生成和 10～50 章长篇细纲的稳定性。
2. 将作品、章节和文风档案从 `localStorage` 迁移到 SQLite，并保留 PostgreSQL 适配方向。
3. 接入通义万相 `wanx2.0-t2i-turbo` 封面生成。
4. 实现长期小说记忆和一致性检查；PDF、EPUB 留待后续。
